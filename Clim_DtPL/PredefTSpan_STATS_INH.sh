...
OUTSTAT_TEMPSCALE="dd_SUM_mm_MEAN_STAT"

KEYSTAT_STEPS="ddVAR1"
...
doPredefTSpanStats(){
  ...
  declare -A ddSTAT_arr=(["ddMEAN"]="daymean" ["ddSUM"]="daysum" ["ddMAX"]="daymax" ["ddMIN"]="daymin" ["ddRNG"]="dayrange" ["ddSTD"]="daystd" ["ddSTD1"]="daystd1" ["ddVAR"]="dayvar" ["ddVAR1"]="dayvar1")
  declare -A ddMYSTAT_arr=(["ddMYMEAN"]="ydaymean" ["ddMYMIN"]="ydaymin" ["ddMYSUM"]="ydaysum" ["ddMYMAX"]="ydaymax" ["ddMYAVE"]="ydayavg" ["ddMYRNG"]="ydayrange" ["ddMYSTD"]="ydaystd" ["ddMYSTD1"]="ydaystd1" ["ddMYVAR"]="ydayvar" ["ddMYVAR1"]="ydayvar1")
    ...
    #check a file is of type dictory
	 ...
	 for i in "${PAR_arr[@]}"
	   do
	     ...
	     temp2=$REG_NAME"-"$i"-"$file"-"$OUTSTAT_TEMPSCALE".nc"
	     OutFIle=$REG_NAME"-"$i"-"$file"-"$OUTSTAT_TEMPSCALE"-"$STT_DATE"-"$END_DATE".nc"
