      ...
      if [ -d $DIR_PATH/$file/ ];
	    ...
	    do
	      ...
	      echo $PAR_NAME
                  ...
                  OutFIle=$PAR_NAME"-"$REG_NAME"-"$file"-"$STAT_TSCALE"-"$STT_DATE"-"$END_DATE".nc"

                  /usr/bin/cdo -seldate,$STT_DATE,$END_DATE $i $temp2

	      echo  "started from $ORIG_TSCALE time scale"
                  echo "****************cdo: getting  $STAT_TSCALE from $ORIG_TSCALE data in $i*****************************"
 
	      echo "started from $ORIG_TSCALE time scale"
                  echo "****cdo: getting dailly $STAT_STEPS of $i from $ORIG_TSCALE data in $file***************"
 
	      if [[ ( "$ORIG_TSCALE" == "dailly" ) && ("$STAT_TSCALE" == "dd_MYRUN_PCTL") ]];
                    then                            
