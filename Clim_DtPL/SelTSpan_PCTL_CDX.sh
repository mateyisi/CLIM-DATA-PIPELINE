  ...
  for file in $foldernames 
    do
	  ...
	  INF=$i
              ...
              temp2min=$PAR_NAME"-"$REG_NAME"-"$file"-"$AGGR_T0"min.nc"
              temp2max=$PAR_NAME"-"$REG_NAME"-"$file"-"$AGGR_T0"max.nc"
              ...
              echo "****************cdo: getting daily mean of $PAR_NAME from $ORIG_TSCALE data in $i*****************************"
...
#doSelTSpanPCTL "FILES[@]" "PARAM[@]" "$PATH" "$OUTFILE" "$OUTFILESUB" "$STARTDATE" "$ENDDATE" "$REG_NAME" "$ORIG_TSCALE" "$OUTSTAT_TEMPSCALE" "$KEYSTAT_STEPS" "$NSETS" "$OFFSETN" "$PCNTL"
