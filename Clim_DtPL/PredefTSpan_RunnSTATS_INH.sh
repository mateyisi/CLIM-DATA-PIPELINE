...
#OUTSTAT_TEMPSCALE="dd_SUM_MYRUN_STAT "

#KEYSTAT_STEPS="ddMYRUNMIN"
     ...
     if [ -d $DIR_PATH/$file/ ];       
	  ...
	  do
	    ...
	    echo "started from $ORIG_TSCALE time scale"
	          ...
	          /usr/bin/cdo -b 64 -$dd_MYRUNSTAT_ar[$STAT_STEPS]},$ddT_STEPS -shifttime,1sec -daysum -shifttime,-1sec $temp2 $OutFIle
                              
                    ;; dd_MEAN_MYRUN_STAT )

                      /usr/bin/cdo -b 64 -${dd_MYRUNSTAT_ar[$STAT_STEPS]},$ddT_STEPS -shifttime,1sec -daymean -shifttime,-1sec $temp2 $OutFIle

                    ;; RUN_STAT )

                     /usr/bin/cdo -b 64 -shifttime,1sec -${RUNSTAT_arr[$STAT_STEPS]},$ddT_STEPS -shifttime,-1sec $temp2 $OutFIle
	        ...
	        case $STAT_TSCALE in dd_MYRUN_STAT )
                      ...
                      /usr/bin/cdo -b 64 -${dd_MYRUNSTAT_arr[$STAT_STEPS]},$ddT_STEPS $temp2 $OutFIle
                              
	        esac
    
                else

                   case $STAT_TSCALE in RUN_STAT )
           
                     /usr/bin/cdo -b 64 -${RUNSTAT_arr[$STAT_STEPS]},$ddT_STEPS $temp2 $OutFIle
...
#doPredefTSpanRunnSTATS "FILES[@]" "PARAM[@]" "$PATH" "$OUTFILE" "$OUTFILESUB" "$STARTDATE" "$ENDDATE" "$REG_NAME" "$ORIG_TSCALE" "$OUTSTAT_TEMPSCALE" "$KEYSTAT_STEPS" "$DAILLYTIMESTEPS"
